...
class ErasureDetector:
    def __init__(self, sr=16000, clip_thresh=0.97, onset_db_thresh=20,
                 kurtosis_thresh=10, min_hold_ms=5, recovery_db_thresh=12, max_hold_ms=80,peak_onset_margin_db=10):
        self.sr = sr
        ...
        self.peak_onset_margin_db = peak_onset_margin_db
    ...
    def process_block(self, x_block, ref_block=None):
        ...
        avg_onset_trigger = onset_db > self.onset_db_thresh

        # peak-based check: catches very brief, isolated spikes that get
        # "averaged away" by the block-mean check above. Needs a stricter
        # (higher) bar since it's judged from a single sample, which is
        # naturally noisier than a whole-block average.
        peak_energy = np.max(x_block ** 2) + 1e-12
        peak_onset_db = 10 * np.log10(peak_energy / (self.slow_mean + 1e-12))
        peak_onset_trigger = peak_onset_db > (self.onset_db_thresh + self.peak_onset_margin_db)

        onset_trigger = avg_onset_trigger or peak_onset_trigger
